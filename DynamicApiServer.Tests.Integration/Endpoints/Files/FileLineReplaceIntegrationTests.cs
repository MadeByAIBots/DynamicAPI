...
namespace DynamicApiServer.Tests.Integration.Endpoints.Files
{
    public class FileLineReplaceIntegrationTests
    {
        ...
        public async Task TestFileLineReplaceEndpoint()
        {
            ...
            var response = await context.Client.PostAsync($"/file-line-replace", new StringContent("{ \"working-directory\": \"" + workingDirectory + "\", \"file-path\": \"" + filePath + "\", \"line-number\": \"" + lineNumber + "\", \"new-content\": \"" + newContent + "\" }", Encoding.UTF8, "application/json"));
