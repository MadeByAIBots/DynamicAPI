...
namespace DynamicApiServer.Tests.Integration.Endpoints.Files
{
	public class FileReadLinesIntegrationTests
	{
		...
		public async Task TestFileReadLinesEndpoint()
		{
			...
			var hashes = lines.Select(GenerateSimpleHash).ToArray();
		...
		private string GenerateSimpleHash(string line)
		{
			var trimmedLine = line.Trim();
			var hash = GetSha1Hash(trimmedLine);
			var shortHash = hash.Substring(0, Math.Min(3, hash.Length));
			//logger?.LogInformation($"Hash generated by GenerateSimpleHash: '{shortHash}'");
			return shortHash;
		}
