...
namespace DynamicApiServer.Tests.Integration.Endpoints.Files
{
    public class FileLineInsertIntegrationTests
    {
        ...
        public async Task TestFileLineInsertEndpoint()
        {
            ...
            // Exercise
            var response = await context.Client.PostAsync($"/file-line-insert", new StringContent("{ \"workingDirectory\": \"" + workingDirectory + "\", \"file-path\": \"" + filePath + "\", \"before-line-number\": \"3\", \"new-content\": \"New line\" }", Encoding.UTF8, "application/json"));
